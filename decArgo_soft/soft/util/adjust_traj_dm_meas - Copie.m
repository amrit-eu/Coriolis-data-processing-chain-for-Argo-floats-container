...
DIR_INPUT_NC_PROF_FILES = 'F:\snapshot-202405\snapshot-202405_arvor_ir_in_andro_327\';

% top directory of the input TRAJ NetCDF files
DIR_INPUT_NC_TRAJ_FILES = 'C:\Users\jprannou\_DATA\OUT\nc_output_decArgo_traj_dm\';
...
GENERATE_OUTPUT_CSV_DETAILED_FLAG = 1;
            ...
            for measCode = mcList'
               ...
               switch (measCode)
                  ...
                  case { ...
                     ...
                     % end
                     % 
                     % % PRES only or JULD + PRES
                     % idNoDef = find( ...
                     %    (trajParamData ~= paramInfo.fillValue) & ...
                     %    (cycleNumber == cyNum) & ...
                     %    (measurementCode == measCode));
                     % 
                     % trajDataAdj = [{cyNum} {paramName} {measCode} {idNoDef} {profCur.presAdjNullFlag} {[]} {profPrev} {profCur} {juldBest(idNoDef)} {double(trajParamData(idNoDef))}];
                     % trajDataAdjAll(cptTrajDataAdjItem, :) = trajDataAdj;
                     % cptTrajDataAdjItem = cptTrajDataAdjItem + 1;
                     % if (cptTrajDataAdjItem > length(trajDataAdjAll))
                     %    trajDataAdjAll = cat(1, trajDataAdjAll, repmat(cell(1, 10), TRAJ_DATA_ITEM, 1));
         ...
         switch paramName
            case {'PRES', 'TEMP'}
               ...
               if (traj.paramAdjNull(idP) == 1)
                  ...
                  traj.dataAdjQc = cat(2, traj.dataAdjQc, repmat(g_decArgo_qcStrGood, size(trajDataAdjAll{idF, end})));
                  if (strcmp(paramName, 'PRES'))
            ...
            case 'PSAL'
               ...
               elseif (traj.paramAdjNull(idP) == -1)
                  ...
                  switch (measCode)
                     ...
                     case {g_MC_DescProf, g_MC_DescProfDeepestBin, g_MC_AscProfDeepestBin, g_MC_AscProf}

                        % retrieve the PTS measurement in the DM profile
                        idTrajPres = find(strcmp(traj.paramList, 'PRES'));
                        idTrajTemp = find(strcmp(traj.paramList, 'TEMP'));
                        idTrajPsal = find(strcmp(traj.paramList, 'PSAL'));
                        if (~isempty(idTrajPres) && ~isempty(idTrajTemp) && ~isempty(idTrajPsal))
                           profCur = traj.profCur;
                           idProfPres = find(strcmp(profCur.paramList, 'PRES'));
                           idProfTemp = find(strcmp(profCur.paramList, 'TEMP'));
                           idProfPsal = find(strcmp(profCur.paramList, 'PSAL'));
                           if (~isempty(idProfPres) && ~isempty(idProfTemp) && ~isempty(idProfPsal))
                              ...
                              if (~isempty(trajDataAdjAll{idF, 9}) && ~isempty(profCur.timeData))
                                 ...
                                 for idL = 1:size(profCur.data, 1)
                                    profMeasStr{idL} = sprintf('%s %.1f %.3f %.3f', ...
                                       ...
                                       profCur.data(idL, idProfPres), ...
                                       profCur.data(idL, idProfTemp), ...
                                       profCur.data(idL, idProfPsal));
                                 end
                                 ...
                                 for idL = 1:size(traj.data, 1)
                                    trajMeasStr{idL} = sprintf('%s %.1f %.3f %.3f', ...
                                       ...
                                       traj.data(idL, idTrajPres), ...
                                       traj.data(idL, idTrajTemp), ...
                                       traj.data(idL, idTrajPsal));
                                 end
                              else
                                 % use PTS only
                                 profMeasStr = cell(size(profCur.data, 1), 1);
                              ...
                              for idL = 1:size(traj.data, 1)
                                 ...
                                 if (length(idLev) == 1)
                                    traj.dataQc(idL, idTrajPres) = profCur.dataQc(idLev, idProfPres);
                                    traj.dataAdj(idL, idTrajPres) = profCur.dataAdj(idLev, idProfPres);
                                    traj.dataAdjQc(idL, idTrajPres) = profCur.dataAdjQc(idLev, idProfPres);
                                    traj.dataAdjErr(idL, idTrajPres) = profCur.dataAdjErr(idLev, idProfPres);

                                    traj.dataQc(idL, idTrajTemp) = profCur.dataQc(idLev, idProfTemp);
                                    traj.dataAdj(idL, idTrajTemp) = profCur.dataAdj(idLev, idProfTemp);
                                    traj.dataAdjQc(idL, idTrajTemp) = profCur.dataAdjQc(idLev, idProfTemp);
                                    traj.dataAdjErr(idL, idTrajTemp) = profCur.dataAdjErr(idLev, idProfTemp);

                                    traj.dataQc(idL, idTrajPsal) = profCur.dataQc(idLev, idProfPsal);
                                    traj.dataAdj(idL, idTrajPsal) = profCur.dataAdj(idLev, idProfPsal);
                                    traj.dataAdjQc(idL, idTrajPsal) = profCur.dataAdjQc(idLev, idProfPsal);
                                    traj.dataAdjErr(idL, idTrajPsal) = profCur.dataAdjErr(idLev, idProfPsal);
                                 else
